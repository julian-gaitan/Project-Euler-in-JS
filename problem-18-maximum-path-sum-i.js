...
function maximumPathSumI(array) {
    maxSum = 0;
    findPathSum(array, 0, 0, 0);
    return maxSum;
}

function findPathSum(array, heightPos, widthPos, accumulated) {
    if (array[heightPos]) {
        accumulated += array[heightPos][widthPos];
        findPathSum(array, heightPos+1, widthPos, accumulated);
        findPathSum(array, heightPos+1, widthPos+1, accumulated);
    } else {
